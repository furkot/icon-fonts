...
function writeResults(toDir, name, { pixelRatio, layout, image }) {
  ...
  const spriteSrc = path.join('sprite', `${basename}.webp`);
  const imgPath = path.join(toDir, spriteSrc);
  const jsonPath = path.join(toDir, 'sprite', `${basename}.json`);

  const tasks = [writeFile(jsonPath, JSON.stringify(layout, null, 2)), writeFile(imgPath, image)];
...
async function writeHtml(toDir, spriteName, layout, spriteSrc) {
  const images = Object.entries(layout).map(([name, { x, y, width, height }]) => ({
    name,
    position: `-${x}px -${y}px`,
    width: `${width}px`,
    height: `${height}px`
