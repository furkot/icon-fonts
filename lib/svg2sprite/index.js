...
const { addPatternDefaults } = require('./pattern.js');
const { makeSvg, resizeSvg } = require('./svg.js');
const { writeResults } = require('./write.js');
const { directory, bag } = require('./directory.js');

module.exports = svg2sprite;

async function svg2sprite(toDir, templateOptions) {
  const { dirname, colorScheme, sprite, svgPath, svgSpritePath, iconDim } = templateOptions;
  const spriteDirectory = directory({
    root: dirname,
    dir: svgSpritePath
  });
  sprite.forEach(s => {
    const { colorScheme: cs, svgSourceFiles = [] } = s;
    if (cs) {
      ...
      Object.assign(s, {
        ...
        iconGroups: colorScheme[cs].iconGroups.map(group => {
          return {
            ...group,
            icons: group.icons.flatMap(icon => nameMap[icon] || icon)
          };
        })
      });
    }
  ...
  async function generateSprite(sprite) {
    ...
    async function* yieldIcon({ iconGroups, pattern: defaultPattern }) {
      // jshint ignore:line
    ...
    async function resolvePattern({ backdrop, fill, margin }) {
      const resolved = await Promise.all(
        backdrop.map(async b => {
          const { svg } = await spriteDirectory.load(b.name, { parse: true });
          const [, , width, height] = svg.$.viewBox.split(' ').map(Number.parseFloat);
          return {
            ...b,
            path: svg.path[0].$.d,
            dim: { width, height }
