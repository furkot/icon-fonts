...
async function svg2sprite(toDir, templateOptions) {
  ...
  async function generateSprite(sprite) {
    const { name, pixelRatio, size } = sprite;
    ...
    async function* yieldIcon({ iconGroups, pattern: defaultPattern }) { // jshint ignore:line
      for (const { icons, pattern: groupPatern } of iconGroups) {
        ...
        for (const id of icons) {
          ...
          const img = makeSvg({ id, svg }, backdrop, { iconDim, iconScaleFactor });
          for(const s of size) {
            const width = s + 2 * backdrop.margin;
            const dim = {
              width,
