...
async function svg2js(from, templateOptions) {
  ...
  const glyph = Object.fromEntries(
    Object.entries(json.svg.defs.font.glyph).map(([key, value]) => {
      const g = value.$;
      ...
      return [name, { path: g.d, width }];
    })
  );

  await fs.writeFile(to, JSON.stringify(result, null, 2));

  return svg2sprite(path.dirname(from), glyph, templateOptions);
}
