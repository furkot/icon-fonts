...
const { convert, xml2json } = require('./util');
...
async function svg2js(from, { paths = [], iconDim, sprite }) {
  ...
  if (paths.length) {
    ...
    const files = [
      fs.writeFile(`${prefix}-icons.json`, JSON.stringify(paths, null, 2))
    ];
  ...
  paths = paths.reduce((result, p) => { result[p] = true; return result; }, Object.create(null));

  const xml = await fs.readFile(from);
  const json = await xml2json(xml);

  const result = {};
  Object.entries(json.svg.defs[0].font[0].glyph).forEach(([key, value]) => {
    const g = value.$;
