...
module.exports = callbackify(svg2sprite);
...
async function svg2sprite(toDir, glyph, templateOptions) {
  ...
  async function createExtras(name, backdrops) {
    ...
    async function readSvg(f) {
      let svg = await fs.readFile(f, 'utf-8');
      const doc = await xml2json(svg);
      const { width, height, viewBox } = doc.svg.$;
      const dim = {
        width: parseInt(width, 10),
        height: parseInt(height, 10)
      };
      if (!viewBox) {
        // add missing viewBox since we are going to be scaling image by manipulating width and height
        const value = `0 0 ${width} ${height}`;
        svg = svg.replace('>', ` viewBox="${value}">`);
      }
      return { dim, svg };
    }
  }
